...
sealed class MyVector<T> : IEnumerable<T>, ICloneable
{
   ...
   public void Push(T item, int count = 1)
   {
       ...
       {
           int newCap = (count == 1) ? _capacity * 2 : _capacity * 2 + count;
           if (_capacity == 0) newCap = count;
           Resize(newCap);
       }
   ...
   private void ResizeWithIndex(int newSize, int index, T item, int count)
   {
       ...
       for (int i = index; i < _size - index ; i++) 
       {
           newArray[i] = _array[i];
       }
...
class Program
{
    static void Main()
    {
        ...
        Console.WriteLine("New top " + stack.Peek());         
        Console.WriteLine("Stack empty?: " + stack.Empty());           

        Console.WriteLine("Popping remaining elements...");
        Console.WriteLine(stack.Pop());  
        Console.WriteLine(stack.Pop());  

        Console.WriteLine("Stack empty now?: " + stack.Empty());       
        ...
        Console.WriteLine("Vector after inserting 'X' at index 1:");
        vector.Print();  

        Console.WriteLine("Peeking last element: " + vector.Peek());  
        Console.WriteLine("Popping last element: " + vector.Pop());   

        Console.WriteLine("after pop:");
        vector.Print(); 
        var clone = (MyVector<string>)vector.Clone();
        Console.WriteLine("Cloned vector:");
        clone.Print();   

        Console.WriteLine("Clearing original vector...");
        vector.Clear();
        Console.WriteLine();
        vector.Print();  
